...
function update(url, visited = new Set(), path = []) {
	...
	if (!accept.length) {
		...
		path.push(url.replace(location.origin, ''));
		if (dependents.size) {
			for (const dependent of dependents) {
				if (!visited.has(dependent)) {
					promises.push(update(dependent, visited, [...path]));
				}
			}
			return promises;
		} else {
		...
		const newUrl = url + '?t=' + Date.now() + `&chain=${path.join(',')}`;
